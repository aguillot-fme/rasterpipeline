...
# --- Changed imports for HDBSCAN ---
# HDBSCAN is not part of scikit‐learn. Import it from the hdbscan package.
...
class Clustering:
    ...
    def estimate_eps(pc: PointCloud, k: int = 4) -> float:
        ...
        # Note: np.percentile sorts internally, so an explicit sort is not needed.
        eps = np.percentile(kth_distances, 90)
        logger.info(f"Estimated eps for DBSCAN: {eps}")
        return eps

    @staticmethod
    def dbscan_clustering(pc: PointCloud,
                          ...
                          auto_eps: bool = False,
                          min_cluster_size: Optional[int] = 1000) -> Tuple[np.ndarray, Dict[int, int]]:
        # --- Changed eps logic: always auto-estimate if eps is None ---
        if eps is None:
    ...
    def hdbscan_clustering(
        ...
        # --- Changed: Import HDBSCAN from the hdbscan package ---
        import hdbscan
        hdb = hdbscan.HDBSCAN(
    ...
    @staticmethod
    def optics_clustering(pc: PointCloud, min_samples: int = 10, max_eps: float = np.inf, metric: str = "minkowski", p: float = 2, cluster_method: str = "xi", eps: float = None, xi: float = 0.05, predecessor_correction: bool = True, min_cluster_size: Optional[int] = None, algorithm: str = "auto", leaf_size: int = 30) -> Tuple[np.ndarray, Dict[int, int]]:
        # --- Removed the "workers" parameter because OPTICS in sklearn does not support it ---
        optics = OPTICS(
            min_samples=min_samples,
            max_eps=max_eps,
            metric=metric,
