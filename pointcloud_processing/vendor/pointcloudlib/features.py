...
class GeometricFeatures:
    ...
    def compute_eigenvalues_and_normals(pc: PointCloud, k_neighbors: int) -> None:
        ...
        # Use the eigenvector corresponding to the smallest eigenvalue as the normal.
        normals = eigvec1
    ...
    @staticmethod
    def compute_geometric_features(pc: 'PointCloud', k_neighbors: int = 20) -> None:
        """
        Convenience wrapper to compute the common geometric feature set
        used across classification/registration utilities.
        """
        GeometricFeatures.compute_eigenvalues_and_normals(pc, k_neighbors=k_neighbors)
        if (pc.get_attribute('eig1') is not None and pc.get_attribute('eig2') is not None and pc.get_attribute('eig3') is not None):
            if (pc.get_attribute('curvature') is None or pc.get_attribute('linearity') is None or pc.get_attribute('planarity') is None):
    ...
    def detect_keypoints_from_features(pc: PointCloud, 
                                       threshold: float = 0.1, 
                                       feature_name: str = "curvature") -> np.ndarray:
        """
        Detects keypoints based on a specified feature threshold.
        
        Parameters:
            pc (PointCloud): The input point cloud.
            threshold (float): The threshold value to consider a point as a keypoint.
            feature_name (str): The name of the feature attribute to use (default: "curvature").
        
        Returns:
            np.ndarray: Indices of detected keypoints.
        """
        attr = pc.get_attribute(feature_name)
        if attr is None:
            # If the requested feature is "curvature", try to compute it from eigenvalues.
            if feature_name == "curvature":
                ...
                attr = eig1 / (eig1 + eig2 + eig3 + 1e-9)
                pc.set_attribute("curvature", attr)
            else:
                raise ValueError(f"Feature '{feature_name}' not found in the point cloud.")
        keypoints = np.where(attr > threshold)[0]
        logger.info(f"Detected {len(keypoints)} keypoints based on {feature_name} threshold {threshold}.")
        return keypoints
...
class StatisticalFeatures:
    ...
    @staticmethod
    def compute_multiscale_roughness_and_curvature(pc: PointCloud,
                                                   voxel_sizes: List[float],
                                                   min_points_per_voxel: int = 3
                                                   ) -> Dict[float, Dict[str, np.ndarray]]:
        """
        Compute roughness and curvature for each point at multiple scales using voxelization.
        
        For each specified voxel size, the point cloud is partitioned into voxels. For each voxel that
        contains at least `min_points_per_voxel` points, the roughness is computed as the standard deviation
        of the z-values, and curvature is computed via a PCA-based measure (smallest eigenvalue divided by the sum
        of eigenvalues). The computed values for each voxel are then assigned to all points within that voxel.
        
        Parameters:
            pc (PointCloud): The input point cloud.
            voxel_sizes (List[float]): A list of voxel sizes (scales) at which to compute the features.
            min_points_per_voxel (int): Minimum number of points required in a voxel to compute features.
            
        Returns:
            Dict[float, Dict[str, np.ndarray]]: A dictionary mapping each voxel size to a dictionary with keys
            "roughness" and "curvature" containing arrays (of length equal to the number of points) of the computed
            values.
            
        Additionally, for each voxel size, the method sets the following attributes in the point cloud:
            - "roughness_scale_{voxel_size}"
            - "curvature_scale_{voxel_size}"
        """
        results = {}
        points = pc.points  # (N, 3)
        n_points = points.shape[0]
        
        # Loop over the desired voxel sizes (scales)
        for voxel_size in voxel_sizes:
            # Compute voxel indices for each point based on the minimum coordinate in the point cloud
            min_coords = points.min(axis=0)
            voxel_coords = np.floor((points - min_coords) / voxel_size).astype(np.int32)
            
            # Get unique voxel identifiers and an inverse mapping
            unique_voxels, inverse = np.unique(voxel_coords, axis=0, return_inverse=True)
            
            # Initialize arrays to hold roughness and curvature per point
            roughness_array = np.zeros(n_points, dtype=np.float32)
            curvature_array = np.zeros(n_points, dtype=np.float32)
            
            # Loop over each unique voxel and compute the features if enough points are present
            for i in range(unique_voxels.shape[0]):
                # Find indices of points that fall into the i-th voxel
                indices = np.where(inverse == i)[0]
                if indices.size < min_points_per_voxel:
                    continue  # Skip voxel if not enough points for a robust estimate
                voxel_points = points[indices]
                
                # Compute roughness as the standard deviation of z-values
                rough = np.std(voxel_points[:, 2])
                
                # Compute curvature using a PCA approach:
                # Calculate the covariance matrix of the voxel’s points (using rowvar=False since each row is a sample)
                cov = np.cov(voxel_points, rowvar=False)
                # Compute eigenvalues (using eigvalsh which returns them in ascending order)
                eigvals = np.linalg.eigvalsh(cov)
                sum_eig = eigvals.sum()
                if sum_eig > 0:
                    curv = eigvals[0] / sum_eig
                else:
                    curv = 0.0
                
                # Assign the computed roughness and curvature to all points in the voxel
                roughness_array[indices] = rough
                curvature_array[indices] = curv
            
            # Store the computed arrays in the results dictionary (keyed by voxel size)
            results[voxel_size] = {"roughness": roughness_array.copy(),
                                   "curvature": curvature_array.copy()}
            
            # Also save the results as attributes in the point cloud for later use
            attr_name_r = f"roughness_scale_{voxel_size}"
            attr_name_c = f"curvature_scale_{voxel_size}"
            pc.set_attribute(attr_name_r, roughness_array)
            pc.set_attribute(attr_name_c, curvature_array)
            logger.info(f"Computed multiscale roughness and curvature for voxel size {voxel_size}.")
...
class FeatureAggregation:
    ...
    def aggregate_features(pc: PointCloud, feature_names: List[str]) -> np.ndarray:
        # Initialize an empty list to hold each feature as a column vector.
        feature_arrays = []
        
        # Loop through the list of feature names.
        for fname in feature_names:
            # Retrieve the attribute corresponding to the feature name.
            attr_data = pc.get_attribute(fname)
            if attr_data is None:
                # If the attribute is missing, raise an error.
                raise ValueError(f"Required feature '{fname}' not found in the point cloud.")
            # Reshape the attribute to a column vector and append it.
            feature_arrays.append(attr_data.reshape(-1, 1))
        
        # Horizontally stack all the column vectors to form a 2D feature matrix.
        feature_matrix = np.hstack(feature_arrays)
        
        # Log the shape of the aggregated feature matrix.
